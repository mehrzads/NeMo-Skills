...
def load_diversity_prompts(
    prompts_path: str, prompt_key: str = "prompt", answer_regex_key: str = "answer_regex"
) -> List[Dict]:
    ...
    for i, prompt_obj in enumerate(prompts):
        ...
        normalized_prompts.append({"prompt": prompt_obj[prompt_key], "answer_regex": prompt_obj[answer_regex_key]})

    return normalized_prompts


def map_prompts_to_dataset(
    input_file: str,
    output_file: str,
    prompts_path: str,
    prompt_key: str = "prompt",
    answer_regex_key: str = "answer_regex",
    seed: int = None,
):
...
def main():
    parser = argparse.ArgumentParser(description="Map random prompts and answer regex patterns to dataset samples")
    parser.add_argument("input_file", type=str, help="Input JSONL dataset file")
    parser.add_argument("output_file", type=str, help="Output JSONL file with prompt and answer_regex fields added")
    parser.add_argument(
        "--prompts_path", type=str, required=True, help="Path to JSON file containing diversity prompts"
    )
    parser.add_argument(
        "--prompt_key", type=str, default="user_prompt", help="Field name for the prompt text in the prompts file"
    )
    parser.add_argument(
        "--answer_regex_key",
        type=str,
    ...
    parser.add_argument("--seed", type=int, default=None, help="Random seed for reproducible prompt assignment")
