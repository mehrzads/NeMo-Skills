    ...
    *,
    ...
    try:
        result = subprocess.run(
            [str(val_binary_path)], input=test_data, capture_output=True, text=True, timeout=timeout
        )
    ...
    *,
    ...
    for val_binary in val_binaries:
        result = run_validator(
            val_binary, test_data, loop=loop, sandbox=sandbox, input_path_in_sandbox=sandbox_input_path
        )
...
def generate_datasets_for_problem(
    problem_dir, binary_dir, output_dir, n_datasets, min_validators=7, generator_workers=4
):
...
def main():
    ...
    parser.add_argument(
        "--min-validators", type=int, default=7, help="Minimum number of validators that must pass (default: 7)"
    )
    parser.add_argument("--base-dir", type=str, default=None, help="Base directory (default: current directory)")
    parser.add_argument(
        "--workers-problems",
        type=int,
