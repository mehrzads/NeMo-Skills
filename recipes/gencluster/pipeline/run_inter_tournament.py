...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="longest",
        help="Selection strategy passed to run_tournament_all.py (default: wins)",
    )
    ...
    if stage == "submission":
        ...
        if benchmark == "ioi":
            genrate_submission_command += f" python /nemo_run/code/gencluster/scripts/submission_IOI.py --ioi25     --intra-strategy={intra_strategy} --inter-strategy={inter_strategy} --inter-strategy={inter_strategy}  --data-subdir={cluster_folder}/cluster_with_scores_{selection_strategy}_reps     --all    --remove-empty &> {initial_cluster_folder}/submission_report.txt;"
        elif benchmark == "icpc":
