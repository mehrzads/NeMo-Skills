...
class ParallelGenTask(GenerationTask):
    ...
    def load_data(self):
        ...
        print(f"Loaded data: {data}, length: {len(data)}, num_generations: {self.cfg.num_generations}")
        ...
        print(f"Loaded data: {data}, length: {len(data)}, max_samples: {self.cfg.max_samples}")
        return data

    def preprocess_data(self, data):
        """A placeholder for any data preprocessing that needs to be done before generation."""
        LOG.info(f"Preprocessing data: {data[4]}")
        return data
